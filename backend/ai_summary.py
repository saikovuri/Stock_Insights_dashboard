...
def _offline_summary(metrics: dict, sentiment: dict) -> str:
    ...
    if pe:
        # Use S&P 500 historical median ~20-22 as anchor, not fixed thresholds
        if pe < 0:
            signals.append(f"P/E of {pe:.1f} — negative earnings")
        elif pe < 18:
            signals.append(f"P/E of {pe:.1f} — below market average, may indicate value")
        elif pe > 35:
            signals.append(f"P/E of {pe:.1f} — well above market average, priced for growth")
        else:
            signals.append(f"P/E of {pe:.1f} — near market average")
